...
| [Media Generation](media-generation.md) | Image and video generation root providers: OpenAI GPT Image (executable), Google Veo/Nano Banana, ByteDance Seedance/Seedream, Alibaba WAN, xAI Grok Imagine | Pre-integration shortlist with registration map, size-control facts, and open verification items; only OpenAI image actions executable, `utils.video.generate` deferred until a backend connector lands. |
