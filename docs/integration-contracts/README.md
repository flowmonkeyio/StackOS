...
| [Current Connectors](current-connectors.md) | OpenAI Images, xAI Imagine, Firecrawl, Jina, Reddit, DataForSEO, Ahrefs, WordPress, Ghost, sitemap, HTTP | Executable surface audit and provider documentation ledger. |
...
| `utils` | OpenAI Images, xAI Imagine image/video actions, Firecrawl scrape/crawl/map, Jina Reader, Reddit, sitemap, static HTTP connector paths, and the local [mock provider](../integration-testing.md) are executable through `action.run` for one explicit call or `action.execute` in run plans; provider-neutral video generation and async extract/status-depth remain deferred until status and artifact contracts exist. |
