...
| `ftp` | `utils.ftp.directory.list`, `utils.ftp.file.upload`, `utils.ftp.file.download` | `stackos/actions/ftp.py`, `stackos/integrations/ftp.py` | `stackos/plugins/manifest.py` built-in utils file-transfer actions | Password payload plus safe host/port/user/TLS/passive/timeout/encoding config; explicit FTPS protects data with `PROT P`; arbitrary agent-selected local paths are supported without a generated-assets gate. |
