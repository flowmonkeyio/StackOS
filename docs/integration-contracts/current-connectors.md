...
Audit date: 2026-09-23 (Stripe PDF transfer and evidence boundaries; other rows retain prior scope)
...
Alibaba Wan, Kling, Firecrawl, Jina Reader, FTP/FTPS, Amazon S3, Cloudflare DNS, Reddit,
DataForSEO, Serper.dev,
Ahrefs, Google Search Console, Google Analytics 4, Google Tag Manager,
HubSpot, Linear, WordPress, Ghost, sitemap, Trackbooth, Shopify, Stripe, generic HTTP, and the internal Branding
...
| FTP / explicit FTPS | [FTP protocol](https://www.rfc-editor.org/rfc/rfc959), [MLST/MLSD](https://www.rfc-editor.org/rfc/rfc3659), [FTP over TLS](https://www.rfc-editor.org/rfc/rfc4217), [Python ftplib](https://docs.python.org/3/library/ftplib.html) | Username/password with server settings; password is daemon-held | Stateless browse/transfer plus exact DELE, MKD, RMD, and RNFR/RNTO operations; recursive delete requires typed MLSx planning; no monetary budget |
| Amazon S3 | [Canonical StackOS contract and official source ledger](s3.md) | Explicit access key id, secret access key, optional session token, bucket, optional validated prefix path, and a selected SDK-supported AWS S3 region | Executable for one bound general-purpose bucket and optional logical prefix root. Account testing uses a one-object `ListObjectsV2` probe at that root. AWS IAM and bucket policy determine access. Conditional single/multipart writes, bounded prefix operations, version/delete-marker receipts, and non-atomic copy-delete move are mock-verified; a disposable live-bucket smoke remains release evidence |
...
| Stripe | [Stripe contract and official source ledger](stripe.md) | Daemon-held API key; safe account evidence and provider-enforced key permissions | Customer and draft-invoice updates have independent digest/payment-method readback. Finalized invoice/PDF URLs are opt-in reads; explicit PDF download stages actual bytes privately for external custody, with account/project-scoped cleanup. A draft has no Stripe PDF or hosted URL. Pay online link removal is not proven by the API. See the canonical contract for recovery and the unavailable PaymentRecord list. |
...
| `aws-s3` | `utils.s3.directory.list`, `utils.s3.file.upload`, `utils.s3.file.download`, `utils.s3.file.delete`, `utils.s3.directory.create`, `utils.s3.directory.delete`, `utils.s3.path.rename` | `stackos/actions/s3.py`, `stackos/integrations/s3.py`, `stackos/s3_contract.py` | `stackos/plugins/builtin_utils_s3.py`, composed by `stackos/plugins/manifest.py` | Explicit access-key secrets stay daemon-held; safe config binds one general-purpose bucket, a selected SDK-supported AWS S3 region, and an optional validated prefix used as the logical root. AWS IAM and bucket policy determine access. Recursive mutations are bounded, and rename is reported as non-atomic copy plus conditional delete. |
...
| `stripe` | 37 declared `finance.stripe.*` customer/tax-ID/Product/Price/invoice/payment actions; 36 executable, PaymentRecord list deferred | `stackos/actions/stripe.py`, `stackos/actions/stripe_pdf.py`, `stackos/integrations/stripe.py` | `plugins/finance/plugin.yaml`, [Stripe contract](stripe.md) | Daemon-held API key and account-bound refs. Memo edits/clearing, complete customer/invoice custom fields, paginated customer tax IDs and actual verification readback are explicit actions. Finalized PDF download follows provider HTTP(S) destinations without a host allowlist and validates PDF bytes. The API does not establish Pay online link visibility. |
...
- Deferred: `grok-imagine-video-1.5-preview` is not exposed because xAI
  documents it as an image-to-video-only preview model; add it only after a
  dedicated preview-model contract/signoff.
...
- Remaining: Reve does not document a free live credential probe. StackOS
  `account.test` for `reve` is format-only and does not make a billable image
...
only. `account.test` calls the read-only models endpoint so operators can verify
