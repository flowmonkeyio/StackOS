...
| Amazon S3 | [Canonical StackOS contract and official source ledger](s3.md) | Explicit access key id, secret access key, optional session token, bucket, and region | Executable for one bound general-purpose bucket. AWS IAM and bucket policy determine access. One-page `ListObjectsV2`, conditional single/multipart writes, bounded prefix operations, version/delete-marker receipts, and non-atomic copy-delete move are mock-verified; a disposable live-bucket smoke remains release evidence |
...
| `aws-s3` | `utils.s3.directory.list`, `utils.s3.file.upload`, `utils.s3.file.download`, `utils.s3.file.delete`, `utils.s3.directory.create`, `utils.s3.directory.delete`, `utils.s3.path.rename` | `stackos/actions/s3.py`, `stackos/integrations/s3.py` | `stackos/plugins/builtin_utils_s3.py`, composed by `stackos/plugins/manifest.py` | Explicit access-key secrets stay daemon-held; safe config binds one general-purpose bucket and region, while AWS IAM and bucket policy determine access. Prefixes remain flat S3 keys, recursive mutations are bounded, and rename is reported as non-atomic copy plus conditional delete. |
