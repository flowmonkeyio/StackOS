...
| MCP registration | ChatGPT / Codex, Claude Code, Claude Desktop, Gemini CLI, and explicit Hermes profiles use the shared host MCP lifecycle in [`setup.md`](./setup.md). Current local stdio registrations are a no-op. Only selected StackOS-owned stale entries are repaired; unmanaged same-name entries are preserved for review. Package installs register the bundled StackOS launcher and pass a host-specific `--runtime`. Desktop launch completes reconciliation before loading the UI so the first status report observes current state. ChatGPT-bundled Codex is preferred, standalone Codex and `Codex.app` remain compatible, and plugin/direct Codex paths use `--runtime codex`. Hermes is never installed or auto-selected; an explicitly chosen existing profile receives MCP plus only the canonical StackOS skill. Claude Desktop config changes require restarting Claude Desktop. Legacy `~/.claude/mcp.json` StackOS entries are cleanup-only and sibling servers are preserved. No host registration stores a bearer token in client config. |
...
| Browser command | Install/repair refreshes the StackOS-owned `~/.local/bin/stackos.browser` launcher with the current package path and daemon host/port/data/state context. Prepared desktop startup repairs missing/stale owned launchers. Foreign same-name entries remain untouched. Uninstall removes only managed launchers; profiles and session history are preserved. |
