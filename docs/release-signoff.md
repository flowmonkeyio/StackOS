...
| Native gstack browser | The app ships pinned upstream gstack, Bun, browser assets, and a managed global `stackos.browser` launcher. The leading session selector resolves once; all native argv, stdin/stdout/stderr, exit codes, PTY and signals pass through exec. Discovery survives daemon restart; native stop/restart preserves profile cookies/storage. Launcher repair covers app moves, non-default daemon context and foreign entries. | Browser launcher/operation/runtime and install tests, `pnpm --dir desktop check`, and installed-app install/repair/restart proof |
