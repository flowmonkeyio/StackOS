  ...
  <div class="space-y-4">
    <UiPanel class="p-4">
      ...
      <dl class="grid gap-3 text-sm md:grid-cols-3">
        <div>
          ...
          <dd class="font-mono">{{ spec.key }}</dd>
        </div>
        <div>
      ...
      <div class="space-y-4">
        <UiPanel class="p-4">
          ...
          <div class="space-y-3 text-sm">
            <section>
              ...
              <p v-if="inputs.length === 0" class="text-fg-muted">-</p>
              <ul v-else class="space-y-1">
                <li v-for="input in inputs" :key="input.key" class="flex justify-between gap-3">
                  <span class="font-medium">{{ input.name ?? input.key }}</span>
            ...
            <section>
              ...
              <p v-if="outputs.length === 0" class="text-fg-muted">-</p>
              <ul v-else class="space-y-1">
                <li v-for="output in outputs" :key="output.key" class="flex justify-between gap-3">
                  <span class="font-medium">{{ output.name ?? output.key }}</span>
        ...
        <UiPanel class="p-4">
          <UiSectionHeader
            ...
            as="h3"
          />
          <p v-if="contextRequirements.length === 0" class="text-sm text-fg-muted">-</p>
    ...
    <div class="grid gap-4 lg:grid-cols-3">
      <UiPanel class="p-4">
        ...
        <UiJsonBlock :data="sanitizeForDisplay(approvals)" density="compact" max-height="12rem" wrap />
      </UiPanel>
      <UiPanel class="p-4">
        <UiSectionHeader title="Policies" as="h3" />
        <UiJsonBlock :data="sanitizeForDisplay(policies)" density="compact" max-height="12rem" wrap />
      </UiPanel>
      <UiPanel class="p-4">
        <UiSectionHeader title="Learning Hooks" as="h3" />
        <UiJsonBlock :data="sanitizeForDisplay(hooks)" density="compact" max-height="12rem" wrap />
      </UiPanel>
    </div>
