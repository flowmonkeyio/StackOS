...
from stackos.repositories.run_plan_output_contracts import validate_step_expected_outputs
from stackos.repositories.run_plan_state import TERMINAL_STEP_STATUSES
from stackos.repositories.run_plan_views import (
    ApprovalRequestOut,
    RunPlanOut,
    RunPlanReopenOut,
    RunPlanStartOut,
...
def _ensure_no_secrets(value: Any, *, label: str) -> None:
    ...
    if paths:
        raise ValidationError(
            f"{label} must not contain secrets as raw values; use credential_ref for provider "
            "auth or secret.set with an exact $secret_ref marker for action payload values",
            data={"paths": paths[:8]},
        )
    ...
    def _step_out(
        ...
        *,
        ...
        if plan is not None:
            ...
            data.action_execution_guidance = step_action_execution_guidance(
                step=step,
                plan=plan,
            ...
            if step.status == "running":
                ...
                bounded_inputs, inputs_truncated = bounded_step_payload(
                    input_values,
                    recovery="Call runPlan.get with response_mode=raw for complete run inputs.",
                )
                data.input_values_json = bounded_inputs or {}
                data.step_context_json, context_truncated = bounded_step_payload(
        ...
        data.direct_dependency_handoffs = direct_dependency_handoffs(
            step,
