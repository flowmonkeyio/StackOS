...
def _browse(
    ...
    for entry in _list_remote(client, remote_path):
        name = str(entry["name"])
        safe = _is_safe_remote_child(name)
        item = dict(entry)
        item["type"] = _public_entry_type(str(item.get("type") or "unknown"))
...
def _upload(client: Any, request: ActionConnectorRequest, state: _TransferState) -> None:
    ...
    for item in payload["items"]:
        ...
        remote_path = _resolved_remote_path(client, str(item["remote_path"]))
        try:
...
def _upload_directory(
    ...
    *,
    ...
    for entry in entries:
        ...
        try:
            _require_safe_command_path(remote_child, "remote_path")
...
def _upload_file(
    ...
    existing = _remote_stat(client, remote_path)
...
def _download(client: Any, request: ActionConnectorRequest, state: _TransferState) -> None:
    ...
    for item in payload["items"]:
        ...
        local_path = Path(str(item["local_path"]))
        try:
            remote = _remote_stat(client, remote_path)
            if remote is None:
...
def _download_directory(
    ...
    *,
    ...
    identity = _remote_directory_identity(client, remote_dir)
    if identity in ancestry:
    ...
    for entry in _list_remote(client, remote_dir):
        name = str(entry["name"])
...
def _ensure_remote_directory(
    ...
    for part in (item for item in remote_path.split("/") if item):
        ...
        existing = _remote_stat(client, current)
        if existing is None:
            client.mkd(current)
...
def _remote_stat(client: Any, remote_path: str) -> dict[str, Any] | None:
    _require_safe_command_path(remote_path, "remote_path")
    try:
...
def _list_remote(client: Any, remote_path: str) -> list[dict[str, Any]]:
    _require_safe_command_path(remote_path, "remote_path")
    try:
...
def _list_remote_fallback(client: Any, remote_path: str) -> list[dict[str, Any]]:
    ...
    for raw_name in names:
        ...
        stat = _remote_stat(client, path) or {"type": "unknown"}
        entries.append(
            {
                "name": name,
...
def _public_entry_type(value: str) -> str:
...
def _safe_pwd(client: Any) -> str | None:
    try:
        ...
        return None
    resolved = str(value)
    _require_safe_command_path(resolved, "server PWD")
    return resolved
...
def _is_safe_remote_child(name: str) -> bool:
    return bool(
        name
        and name not in {".", ".."}
        and "/" not in name
...
def _require_safe_command_path(value: str, field_name: str) -> None:
    if not value:
