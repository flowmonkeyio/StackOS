"""FIN-GAP001: read-only MCP validation must preserve connector idempotency input."""
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("validate-mcp-stripe-customer-v1", None, id="valid"),
        pytest.param(None, "required", id="missing"),
        pytest.param(" ", "required", id="blank"),
        pytest.param("v" * 256, "max_length", id="overlong"),
    ],
)
def test_mcp_action_validate_forwards_idempotency_without_execution(
    ...
    with Session(engine) as session:
        credential_ref = (
            ...
            .store_credential(
                ...
                display_name="Validator MCP fixture",
                fields={"api_key": "sk_test_validator_mcp_sentinel"},
                attach_project_id=project_id,
            )
            .data.credential_ref
        )
    secret = mcp_client.call_tool_structured(
        "secret.set", {"project_id": project_id, "value": "validator-mcp@example.test"}
    )["data"]["secret_ref"]
    context_ref = "ctx_validator_mcp_stripe"
    ...
    assert created["data"]["context_ref"] == context_ref
    ...
    # Repeating validation must reach the validator again, not consume/replay a key.
    for _ in range(2):
        result = mcp_client.call_tool_structured("action.validate", arguments)
        ...
        assert "sk_test_validator_mcp_sentinel" not in json.dumps(result)
        assert "validator-mcp@example.test" not in json.dumps(result)
